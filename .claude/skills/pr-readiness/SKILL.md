---
name: pr-readiness
description: Validate changes and pull requests through merge readiness, including local gates, independent review, CI, manual evidence, and unresolved review threads.
---
...
4. Run an independent review after the complete local gate passes, using the
   `/code-review` skill (or `pr-review-toolkit:review-pr` for a fuller pass).
   ...
   remain. Explain verified false positives without changing correct code. A
   review-mode pass must report findings directly and never launch a nested
   review. If review tooling is unavailable or fails, report it as a
   readiness blocker instead of silently skipping it.
...
7. For merge readiness, require an independent review of the current change.
   A completed `/code-review` pass can supply that evidence unless the
...
- the independent review loop is incomplete
