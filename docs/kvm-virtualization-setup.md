...
## Redo after reinstall (2026-09-24)

The host was reinstalled as Debian 13 trixie, so everything below this section describes the pre-reinstall `forky/sid` system and its package versions. On the new install, only the article's own steps were redone: `qemu-system-x86` (1:10.0.13), `qemu-utils`, `libvirt-daemon-system` (11.3.0), `libvirt-clients`, `virt-manager`; `libvirtd` enabled and active; `kartik` added to `libvirt`; the `default` NAT network started and autostarted (`virbr0`, 192.168.122.1/24). Non-root `virsh --connect qemu:///system list --all` was verified via `sg libvirt`. `virt-host-validate qemu` passes everything except two harmless warnings: the cgroup `devices` controller (cgroup v2 handles device control through BPF) and "secure guest" support (AMD SEV/Intel TDX only). Not redone: the `ovmf-amdsev`/`ovmf-inteltdx` extras (plain `ovmf` came in as a dependency) and `vm-set-virtio-input.sh`.

Bridging was redone the same day, adapted to the new install, which uses DHCP rather than the old static IP. `br0` (`bridge.stp no`) runs DHCP with `bridge.mac-address a4:1f:72:4d:d9:4e` (`enp3s0`'s own MAC), so the router keeps handing out `192.168.0.222`. It also carries the Cloudflare DNS pin (`1.1.1.1 1.0.0.1`, `ipv4.ignore-auto-dns yes`) and `ipv6.method disabled`. `enp3s0` is enslaved via `br0-port1`. `Wired connection 1` is the rollback profile: kept, with `autoconnect no`, playing the role `enp3s0-rollback` had before. The cutover ran from the local console as one root script that would roll back automatically if the gateway didn't answer through `br0` (log: `/var/tmp/br0-cutover-20260924.log`). It didn't need to. `br0net` was then redefined under `qemu:///system` with the same XML as below. Verified: `br0` holds `.222` and the default route, `bridge link show` lists `enp3s0` as `forwarding`, `resolv.conf` lists only Cloudflare, HTTPS works, and `br0net` and `default` are both active and autostarted. Reboot persistence is only config-inspected: all three profiles are keyfiles under `/etc/NetworkManager/system-connections/`. To roll back, run `nmcli connection delete br0-port1 br0 && nmcli connection modify "Wired connection 1" connection.autoconnect yes && nmcli connection up "Wired connection 1"`.
