...
- `fjord/`: a read-only Astro site (Nord palette) for browsing this repo's own documentation, replacing `productivity/dashboard.html` as the workspace's entry point once built. Has its own `AGENTS.md`/`SPEC.md`/`ROADMAP.md`/`TASKS.md` — that project's docs are the source of truth for its phases from here on, same pattern as `personal-website`/Loopwire had. Started 2026-08-29.
