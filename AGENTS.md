...
7. Run a `/code-review` pass over any non-trivial change before reporting it done. Fix actionable findings, rerun affected validation, and repeat until none remain. Treat an unavailable or failed review as a blocker, not something to silently skip past while still reporting done. When the current task *is* a review, report findings directly — never invoke a nested review from inside one.
