...
- **Local pre-commit hook** (`.githooks/pre-commit`, wired via `git config core.hooksPath .githooks`): lints staged markdown with `mdl`, checks local cross-references for rot (`check-markdown-links.py`), validates staged JSON/YAML config parses cleanly (`validate-config.sh`), and runs `lint-bash.sh`/`lint-python.sh` over staged bash and Python files (bash scripts found by shebang, not extension, since not all of them are `*.sh`). Skips the five reference clones.
- **GitHub Actions** (`.github/workflows/`): four path-filtered `lint-*.yml` files (`lint-markdown.yml`, `lint-config.yml`, `lint-bash.yml`, `lint-python.yml`) each re-run the matching local check as a backstop for a bypassed (`--no-verify`) or elsewhere-made commit — split from a single combined `lint.yml` (2026-09-03) so, e.g., a markdown-only change doesn't also trigger the bash/Python/config checks; file discovery for each category is shared between the local full-sweep script (`lint-markdown.sh`) and these workflows via `list-lint-files.sh`. `gitleaks.yml` scans for secrets on every push/PR. `fjord-ci.yml` runs `astro check` plus a full `astro build` against `fjord/` as a doc-render smoke check, path-filtered to that subfolder. `dependabot.yml` opens weekly PRs for `fjord/`'s npm dependencies.
